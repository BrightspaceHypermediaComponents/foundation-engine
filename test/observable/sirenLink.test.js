...
describe('call sirenLink methods', () => {
	...
	it('SirenLink has link set', () => {
		const link = new SirenLink();
		link.link = 'abc';
		assert.equal(link.link, 'abc', 'SirenLink link set incorrectly');
		assert.equal(link._observers.components.size, 0, 'SirenClass has observer when it should not');
	});
});
...
describe('sirenLink with observers', () => {
	...
	it('SirenLink has an observerMap attached', () => {
		const link = new SirenLink();

		link.addObserver(obs, 'foo', { method });

		const map = link._observers.components;
		assert.equal(map.size, 1, 'SirenLink has incorrect number of observers');
		assert.equal(map.get(obs), 'foo', 'observer maps to incorrect value');
	...
	it('SirenLink with incorrect link has an observerMap attached', () => {
		...
		assert.isTrue(methods.has(obs), 'method for observer is not stored');
		assert.equal(methods.get(obs), method, 'incorrect method stored for observer');
		assert.isUndefined(obs['hello'], 'observer applies method incorrectly');
	});

	it('SirenLink with link has an observerMap attached', () => {
		link.addObserver(obs, 'hello', { method });
	...
	it('SirenLink with link has an observerMap with route attached', () => {
	...
	it('SirenLink with link has an observerMap with route  removed', () => {
	...
	it('SirenLink update route with unique fields', () => {
		...
		const routes = link._routes;
		assert.equal(routes.size, 1, 'incorrect number of routes stored in observer');
		assert.equal(routes.get(obs).route, { name: 'abcde', id: 'fghij' }, 'route set incorrectly for observer');
	});
});
...
describe('merging sirenLinks', () => {
	...
	it('Two SirenLinks with unique components without routes are merged',  () => {
		...
		const map2 = link2._observers.components;
